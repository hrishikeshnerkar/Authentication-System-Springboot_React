...
const ResetPassword = () => {
    ...
    const onSubmitEmail = async (e) => {
        ...
        try {
            ...
            if (err.response) {
                const msg = err.response.data?.message || "Unable to send OTP. Please try again later.";
                toast.error(msg);
            } else {
    ...
    const handleVerify = async () => {
        const otp = inputRef.current.map((input) => input.value.trim()).join("");
        if (otp.length != 6) {
    ...
    return (
        <div className="email-verify-container d-flex align-items center justify-content-center vh-100 position-relative"
            style={{ background: "linear-gradient(90deg, #6a5af9,#8268f9, #8268f9)", border: "none" }}>
            <Link to="/" className="position-absolute top-0 start-0 p-4 d-flex align-items-center gap-2 text-decoration-none">
                <img src={assets.logo} alt="logo" height={32} width={32} />
                <span className="fs-4 fw-semibold text-light">AuthNest</span>
            </Link>

            {/* RESET PASSWORD CARD */}
            {!isEmailSent && (
                <div className="rounded p-5 text-center bg-white" style={{ width: "100%", maxWidth: "400px" }}>
                    ...
                    <form onSubmit={onSubmitEmail}>
                        <div className="input-group mb-4 bg-secondary bg-opacity-10 rounded-pill">
                            ...
                            <input type="email" className="form-control bg-transparent border-0 ps-1 pe-4 rounded-end" placeholder="Enter email Address"
                                style={{ height: "50px" }}
